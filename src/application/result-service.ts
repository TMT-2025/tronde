 ...
 */
export function getJobResult(jobId: string): ExamJobResult {
  ...
  if (job.status !== "COMPLETED" || !job.result) {
    throw new ResultNotFoundError(
      `Tiến trình '${jobId}' chưa hoàn thành (trạng thái hiện tại: ${job.status}).`
    );
  }
 ...
 */
export function getJobZipBuffer(jobId: string): { buffer: Buffer; fileName: string } {
 ...
 */
export function getJobAnswerKey(jobId: string): any {
  const result = getJobResult(jobId);
