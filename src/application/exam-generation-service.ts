...
import { runExamPipeline, ExamPipelineResult } from "../core/pipeline/exam-pipeline.js";
 ...
 */
export async function analyzeSourceDocx(
  buffer: Buffer | Uint8Array,
  fileName: string
): Promise<ExamAnalysisPreview> {
...
export async function executeJobPipeline(
  ...
  onProgress?: (progress: ExamJobProgress) => void
): Promise<ExamJobResult> {
  ...
  try {
    ...
    const pipelineResult: ExamPipelineResult = await runExamPipeline({
    ...
    const translatedError: ExamJobError = {
