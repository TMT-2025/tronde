 ...
 */
export function validateJobConfiguration(config: Partial<ExamJobConfiguration>): ExamJobConfiguration {
...
class ExamJobService {
  private jobs = new Map<string, ExamGenerationJob>();

  /**
   * Creates a new exam generation job in QUEUED status
   */
  public createJob(params: CreateJobParams): ExamGenerationJob {
   ...
   */
  public getJob(id: string): ExamGenerationJob | undefined {
   ...
   */
  public async startJob(id: string): Promise<ExamGenerationJob> {
   ...
   */
  public async executeJobSync(id: string): Promise<ExamGenerationJob> {
    const job = this.jobs.get(id);
    if (!job) {
      throw new Error(`Job '${id}' not found.`);
    }
   ...
   */
  public listJobs(): ExamGenerationJob[] {
