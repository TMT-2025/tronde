...
import { ExamJobFile } from "../application/types.js";

export interface ParsedRequest {
  files: Map<string, { fileName: string; mimeType: string; data: Buffer }>;
 ...
 */
export async function parseRequestBody(req: http.IncomingMessage): Promise<ParsedRequest> {
  const files = new Map<string, { fileName: string; mimeType: string; data: Buffer }>();
...
function splitBufferByBoundary(buf: Buffer, boundary: string): Buffer[] {
 ...
 */
export async function handleExamMixerRequest(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
  // CORS headers
  ...
  const rawUrl = (req.headers["x-forwarded-url"] as string) || req.url || "/";
  const parsedUrl = new URL(rawUrl, `http://${req.headers.host || "localhost"}`);
  let pathname = parsedUrl.pathname;

  // Handle Vercel rewrite mapping if any
  if (pathname === "/api" || pathname === "/api/") {
    const orig = req.headers["x-matched-path"] as string;
    if (orig && orig !== "/api") {
      pathname = orig;
    } else {
      pathname = "/";
    ...
    try {
      ...
      if (req.method === "POST" && pathname === "/api/jobs") {
        ...
        let sourceFile: ExamJobFile;
        let templateFile: ExamJobFile | undefined;
        ...
        if (jsonBody) {
          ...
          if (rawConfig) {
            try { configuration = JSON.parse(rawConfig); } catch {}
          }
        }
 ...
 */
export function createExamMixerServer(): http.Server {
