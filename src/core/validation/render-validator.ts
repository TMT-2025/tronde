...
export async function validateRenderedDocx(
  docxBuffer: Buffer | Uint8Array,
  expectedVariant: VariantExamResult
  ...
  for (const va of vertAligns) {
    ...
    if (val === "subscript") subscriptCount++;
    if (val === "superscript") superscriptCount++;
  }
