...
export function parseRunElement(rElement, options = {}) {
    const runs = [];
    const rPr = getFirstChildByLocalName(rElement, "rPr");
    let isBold = false;
    let isItalic = false;
    let underlineVal = undefined;
    let vertAlign = undefined;
    let fontName = undefined;
    let fontSizeHalfPoints = undefined;
...
export function parseParagraphElement(pElement, options = {}) {
    const runs = [];
    const pPr = getFirstChildByLocalName(pElement, "pPr");
    let alignment = undefined;
    let spacingBeforeDxa = undefined;
    let spacingAfterDxa = undefined;
    let lineSpacingDxa = undefined;
    const tabStops = [];
    if (pPr) {
        ...
        if (jcNode) {
            ...
            if (jcVal === "center" || jcVal === "right" || jcVal === "both" || jcVal === "left") {
                alignment = jcVal === "both" ? "justify" : jcVal;
            }
        }
        ...
        if (spacingNode) {
            ...
            if (before)
                spacingBeforeDxa = parseInt(before, 10);
            const after = getAttributeValue(spacingNode, "after");
            if (after)
                spacingAfterDxa = parseInt(after, 10);
            const line = getAttributeValue(spacingNode, "line");
            if (line)
                lineSpacingDxa = parseInt(line, 10);
        }
...
export function elementHasUnderline(element) {
...
export function elementHasColor(element, colorHexPrefix) {
