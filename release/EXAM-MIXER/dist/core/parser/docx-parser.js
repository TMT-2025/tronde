...
import { getChildrenByLocalName, getFirstChildByLocalName, getAttributeValue, getElementTextContent } from "./xml-utils.js";
import { parseParagraphElement, elementHasUnderline, elementHasColor } from "./run-parser.js";
export async function parseDocx(bufferOrUint8Array, options = {}) {
    ...
    if (sectPrEl) {
        ...
        if (pgSzEl) {
            ...
            if (w)
                pageWidthDxa = parseInt(w, 10);
            if (h)
                pageHeightDxa = parseInt(h, 10);
        }
        ...
        if (pgMarEl) {
            ...
            if (top)
                marginTopDxa = parseInt(top, 10);
            if (bottom)
                marginBottomDxa = parseInt(bottom, 10);
            if (left)
                marginLeftDxa = parseInt(left, 10);
            if (right)
                marginRightDxa = parseInt(right, 10);
        }
    }
    // Collect paragraphs
...
function stripLeadingLabel(p, regex) {
    if (p.runs.length === 0)
        return;
...
function checkForChemicalFormulas(stems, other) {
