 ...
 */
export async function validateUploadedDocx(buffer, fileName, mimeType) {
    const sanitizedName = sanitizeFileName(fileName);
    // 1. Extension check
    const ext = path.extname(sanitizedName).toLowerCase();
    if (ext !== ".docx") {
        throw new FileValidationError("ERR_INVALID_EXTENSION", `Định dạng file không hợp lệ (${ext || "không có phần mở rộng"}). Hệ thống chỉ chấp nhận tệp Microsoft Word (.docx).`);
    }
    // 2. MIME type check (if provided)
    if (mimeType && mimeType.trim().length > 0) {
        const normMime = mimeType.toLowerCase().split(";")[0].trim();
        if (!ALLOWED_MIME_TYPES.has(normMime)) {
            throw new FileValidationError("ERR_INVALID_MIME", `Kiểu nội dung (MIME type: ${normMime}) không được hỗ trợ. Vui lòng tải lên tệp .docx hợp lệ.`);
        }
    }
    // 3. File size check
    ...
    if (size > MAX_FILE_SIZE_BYTES) {
        ...
        throw new FileValidationError("ERR_FILE_TOO_LARGE", `Dung lượng tệp vượt quá giới hạn cho phép (${sizeMb} MB > 50 MB).`);
    }
    ...
    const isZip = buffer[0] === 0x50 &&
        ...
        buffer[3] === 0x04;
    if (!isZip) {
        throw new FileValidationError("ERR_NOT_A_ZIP", "Tệp không đúng định dạng nhị phân OpenXML (thiếu chữ ký ZIP hợp lệ).");
    }
    // 5. Structure check: must contain word/document.xml
    try {
        const zip = await JSZip.loadAsync(buffer);
        if (!zip.file("word/document.xml")) {
            throw new FileValidationError("ERR_MISSING_DOCUMENT_XML", "Tệp .docx bị lỗi cấu trúc: không tìm thấy thành phần word/document.xml bên trong tài liệu.");
        }
    }
    catch (err) {
        if (err instanceof FileValidationError)
            throw err;
        throw new FileValidationError("ERR_CORRUPT_ARCHIVE", `Không thể giải nén tài liệu .docx: ${err.message}`);
    }
 ...
 */
export function getSandboxDirectory(...subPaths) {
 ...
 */
export async function saveUploadedFileToSandbox(jobId, buffer, fileName, mimeType) {
